...
crypto_resolver={
                'Bitcoin':['data_bitc','model'],
                'Ethereum':['ethc','model_ether'],
                'Binance Coin':['binance','model_binance'],
                'Tether':['tether','model_tether'],
                'Cardano':['cardano','model_cardano'],
                'XRP':['xrp','model_xrp'],
                'Dogecoin':['doge','model_doge'],
                'Polkadot':['polka','model_polka'],
...
def cryptoprediction(name):
    if name in list(crypto_resolver):
        ...
        # model.save_weights("home/"+k[1]+".h5")
        # print("Saved model to disk")

        json_file = open("home/"+k[1]+'.json', 'r')
        loaded_model_json = json_file.read()
        json_file.close()
        model = model_from_json(loaded_model_json)
        # load weights into new model
        model.load_weights("home/"+k[1]+".h5")
        ...
        for i in l1[prediction_days+future_day+1:]:
            i=str(i)
            dates.append(i[0:10])
        return list(actual_prices[prediction_days+future_day+1:]),list(prediction_prices[0:,0]),dates,prediction[0,0],R2,MAE
    else:
        return "CryptoCurrency Not present in List"

#print(len(cryptoprediction("Ethereum")));
